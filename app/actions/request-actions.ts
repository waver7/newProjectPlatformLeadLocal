...
export async function createRequestAction(formData: FormData) {
  ...
  const req = await prisma.request.create({
    data: {
      ...
      moderationLogs: { create: { targetType: 'REQUEST', status: moderation.status, reason: moderation.reason, actorUserId: session.user.id } }
    }
  });

  await prisma.clientProfile.update({ where: { userId: session.user.id }, data: { phonePrivate: parsed.data.phonePrivate, emailPrivate: parsed.data.emailPrivate, freePostsUsed: { increment: 1 } } });
...
export async function awardBidAction(requestId: string, bidId: string) {
  const session = await auth();
  if (!session?.user || session.user.role !== 'CLIENT') return { error: 'Unauthorized' };
  const request = await prisma.request.findUnique({ where: { id: requestId }, include: { bids: true } });
  ...
  await prisma.$transaction([
    ...
    prisma.notification.create({ data: { userId: selected.contractorId, type: 'BID_ACCEPTED', title: 'Your bid was accepted', body: `You won the bid for ${request.title}`, href: `/dashboard/contractor/requests/${requestId}` } })
  ]);
  return { ok: true };
}
